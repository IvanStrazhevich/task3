...
public class ParserToParagraph<T> implements SourceParsable<TextDataComposite> {
    ...
    private static final String PARAGRAPH_DIVIDER = "\\r";

    @Override
    public TextDataComponent parseText(String data) {
        String[] paragraphs = data.split(PARAGRAPH_DIVIDER);
        textDataComponent = new TextDataComposite(DataLevel.PARAGRAPH);
